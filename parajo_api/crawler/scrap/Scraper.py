...
class Scrapper:
  ...
  def scrapCarPriceInfo(self):
    ...
    for modelDetail in carDBList:
      ...
      print(f"brand: {brand.name}, model: {model.name},  model-detail:{modelDetail_name} ")
      #스크랩하기
      scrapedResult = crawler.crawlCarPrice(brand.name, model.name, modelDetail_name)
      if scrapedResult is not None:
        self.storeCarInfo(catg_modeldetail_id, scrapedResult)
      # print(result)
    print('END-SCRAPING-CAR-PROCESS =================')
    crawler.getDriver().quit() #끝났으면 셀레니움 브라우져 정상종료
  
  # 차정보DB에 저장
  def storeCarInfo(self, catg_modeldetail_id, scrapedResult):
    for item in scrapedResult:
